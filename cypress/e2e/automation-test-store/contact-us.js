...
describe("Test Contact Us form via Automation Test Store", () => {
  it("Should be able to submit a succesful submission via contact us form", () => {
    ...
    cy.get("a[href$='contact']")
      .click()
      .then(function (btn) {
        cy.log(`Clicked on:${btn.text()}`);
      }); //so css preku extension

    // cy.xpath("//a[contains(@href, 'content/contact')]").click(); so xpath
    // cy.get(".info_links_footer > :nth-child(5) > a").click(); so cypress selektor
